...
def run_autonomous_loop(topic, iterations=1):
    ...
    # Cartella di lavoro per questo argomento (es. surveys/agenti_llm_e_memoria)
    clean_name = re.sub(r'[^a-zA-Z0-9]', '_', topic.lower()).strip('_')
    topic_dir = os.path.join("surveys", clean_name)
    
    # IL FILE SI CHIAMA COME IL TOPIC (es. agenti_llm_e_memoria.md)
    ...
    for i in range(1, iterations + 1):
        ...
        # 5. Esecuzione di Aider (Non interattivo, fa il lavoro e si chiude)
        print("\n[AI AGENT] Passo il controllo a Qwen per lo screening e la scrittura...")
        aider_cmd = [
            "aider",
            "--model", "openai/qwen-3.6",  # <--- NOTA: Modifica se usi un nome/modello diverso in Aider
            "--read", "prepare.py",
            "--read", "program.md",
